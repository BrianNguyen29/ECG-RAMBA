...
class ArtifactSourceAuditTest(unittest.TestCase):
    ...
    def test_notebook02_direct_run_locks_current_oof_and_external_contracts(self):
        ...
        self.assertIn("EXTERNAL_RUN_PROFILE = os.environ.get(", source)
        self.assertIn("EXTERNAL_GATE_DATASETS_DEFAULT", source)
        ...
        self.assertIn("require_gpu_inference_runtime('External Full-model re-export for '", source)
        self.assertIn("require_gpu_inference_runtime('PTB-XL fold 9 Full-model export')", source)
        self.assertIn("PTB-XL fold 9 targeted restore", source)
        self.assertIn("DEFERRED PTB-XL fold 9 export", source)
        self.assertIn("The completed external protocol gates remain valid", source)
        self.assertIn("EXTERNAL_COMPARATOR_DATASETS = 'ptbxl,georgia,cpsc2021'", source)
        self.assertIn("REQUIRE_TRANSFORMER_EXTERNAL_EVIDENCE = True", source)
        self.assertIn("fold{fold}_transformer_ecg_final.pt' for fold in range(1, 6)", source)
        ...
        self.assertIn("True few-shot result targeted restore", source)
