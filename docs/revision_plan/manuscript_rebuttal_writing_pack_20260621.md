...
MiniRocket-only, HRV-only, ResNet1D/CNN, and Raw Mamba baselines are complete under the frozen evidence package. ResNet1D/CNN significantly outperforms Full ECG-RAMBA across the paired in-domain metrics, and Raw Mamba outperforms Full on PR-AUC, ROC-AUC, and F1. The rebuttal must not claim broad fair-baseline superiority or in-domain CNN/ResNet/Mamba superiority.
...
| R1C4 baseline strength | ECG-RAMBA is superior to fair ECG baselines | MiniRocket-only, HRV-only, ResNet1D/CNN, and Raw Mamba are complete. ResNet1D/CNN significantly outperforms ECG-RAMBA on frozen Chapman OOF across the principal metrics; Raw Mamba outperforms ECG-RAMBA on discrimination/F1 metrics. | `complete_baseline_matrix_requires_metric_specific_interpretation` | Do not present broad fair-baseline superiority or in-domain CNN/ResNet/Mamba superiority. |
