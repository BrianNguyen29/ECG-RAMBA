...
- ECG-RAMBA must not be presented as a benchmark-leading, broadly superior, or leading in-domain model.
...
- Georgia/CPSC2021, few-shot adaptation, full HRV, and broad robustness claims remain deferred. Representation probe/CKA is complete, but it supports only a conservative branch-embedding audit and does not establish label-aligned morphology-rhythm separation.
...
The strongest rebuttal strategy is therefore not to defend the original unqualified transfer, strict separation, or benchmark-leading narrative. The revised response should emphasize that the authors accepted the reviewers' concerns, added fair evidence, and narrowed the claims.
...
| R1-C5 Statistical significance/CI | Need CI/significance, especially for external transfer. | Complete for frozen OOF and fair paired comparisons. PTB-XL mapped-task gate includes bootstrap CI. | Current response can cite record-level bootstrap with 1000 resamples and paired bootstrap tables. | Do not claim unqualified external-transfer advantage. If adding Georgia/CPSC/few-shot later, each must have its own bootstrap CI/gate. |
...
| R2-C2 Morphology-rhythm separation/disentanglement | Lead dropout alone is insufficient; need qualitative/quantitative representation evidence. | Complete as a conservative audit. `22_extract_representations.py` produced checkpoint-fingerprinted embeddings and `20_representation_probe.py` produced fold-safe probe/CKA artifacts. Linear probes are weak and CKA only shows branches are not identical. | Current manuscript reports the probe/CKA audit and avoids mechanistic separation claims. | Cite the representation probe/CKA artifacts only as suggestive branch-specific analysis and a limitation, not as proof of label-aligned morphology-rhythm separation. |
...
| R2-C4 Few-shot vs external-transfer comparison | Need comparative simulation between no-target-label and few-shot ECG classification. | Not complete as manuscript evidence. `19_fewshot_adaptation.py` now exists as a gated score-calibration runner with fixed target-domain test splits and nested few-shot pools, but it has not been run on a completed external gate package. | Current response defers few-shot and states that no unqualified external-transfer claim is made. | Run only after an external dataset gate passes. Current runner supports leakage-audited score calibration on frozen predictions; model-weight fine-tuning and matched comparator few-shot runs remain optional future extensions if required. |
...
| PTB-XL external gate | `external_ptbxl_protocol_gate.json` | 2198 records, 4 classes; ROC-AUC 0.8007, PR-AUC 0.6097, F1 0.4623; bootstrap CI present | Protocol-gated mapped-task external evaluation only; not an unqualified external-transfer result. |
...
| P2 if reviewer challenges representation | Representation probe/CKA/UMAP | Complete as a conservative audit; still insufficient for strong mechanistic claims. | Extractor and probe runners exist (`22_extract_representations.py`, `20_representation_probe.py`), Notebook 06 has explicit flags/cache-aware wiring, and final evidence includes probe/CKA outputs. Use cautious "suggestive" wording only. |
...
- "We removed claims of leading in-domain performance and broad fair-baseline advantage."
...
- "Representation probe/CKA is complete as an audit; current analyses still do not establish label-aligned morphology-rhythm separation."

### What not to say

- "ECG-RAMBA is benchmark-leading."
- "ECG-RAMBA is the leading in-domain model."
...
|---|---|---|---|
| Georgia gate | Need additional external mapped-task evidence. | Reviewed SNOMED/Chapman mapping; explicit unmapped-label table; no coercion to negative labels; exporter; protocol gate; bootstrap CI; manifest/checksum. | Dataset-specific mapped-task result only. No unqualified external-transfer claim. |
...
| Representation probe | Reviewer requires stronger support for branch separation. | Complete; verify fold caches, manifest/checksums, probe table, CKA table, and figures when packaging. | Suggestive branch-specific information, not established morphology-rhythm separation. |
