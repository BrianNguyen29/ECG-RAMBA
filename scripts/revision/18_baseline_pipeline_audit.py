...
This script performs static and artifact-level checks for the legacy-named
fixed-seed ROCKET-family head, ResNet1D/CNN, and Raw Mamba same-fold comparator
pipelines. It is intentionally lightweight: it does not train models or import
Mamba runtime code.
"""
...
from scripts.revision.common import METRIC_DIR, TABLE_DIR, save_csv, save_json, sha256_file  # noqa: E402
...
def _summary_check(
    ...
    *,
    ...
    _add(
        ...
        component,
        "oof_sha",
        "pass" if oof_sha == expected_oof_sha256 else "fail",
        str(oof_sha),
    )
    ...
    _add(
        ...
        component,
        "record_fingerprint",
        "pass" if fingerprint == expected_record_fingerprint else "fail",
        str(fingerprint),
    )
    ...
    comparator_contract = summary.get("comparator_contract")
    _add(
        rows,
        component,
        "machine_readable_comparator_contract",
        "pass" if isinstance(comparator_contract, dict) and comparator_contract.get("schema_version") == 1 else "warn",
        (
            "schema_version=1"
            if isinstance(comparator_contract, dict) and comparator_contract.get("schema_version") == 1
            else "missing until baseline is regenerated by the current runner"
        ),
    )
...
def _audit_artifacts(rows: list[dict]) -> None:
    for root in _artifact_roots():
        ...
        oof_path = root / "predictions" / "oof_final_ema_predictions.npz"
        if not oof_path.is_file():
            _add(rows, "artifacts", "canonical_oof_contract", "warn", f"missing under {root}")
            continue
        try:
            with np.load(oof_path, allow_pickle=False) as data:
                fingerprint = str(np.asarray(data["dataset_record_order_fingerprint"]).item())
            oof_sha = sha256_file(oof_path)
        except Exception as exc:
            _add(rows, "artifacts", "canonical_oof_contract", "fail", f"{oof_path}: {exc}")
            continue
        for name, protocol in (
            ("minirocket_only", EXPECTED["minirocket_protocol"]),
            ("resnet1d_cnn", EXPECTED["resnet_protocol"]),
            ("raw_mamba", EXPECTED["raw_mamba_protocol"]),
        ):
            _summary_check(
                rows,
                root,
