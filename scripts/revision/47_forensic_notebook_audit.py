...
def reviewer_traceability_rows(
    ...
    specifications = [
        ...
        ("R2-C4", "Zero-target-label versus few-shot", "02,02a", "02_predictions_and_external_eval.ipynb::cells 32,34,36,38,42,46", "33_group_safe_score_calibration.py;35_true_fewshot_head_adaptation.py;51_ptbxl_adaptation_analysis_lock.py;52_ptbxl_fold_protocol_audit.py", "PTB-XL official fold 9 adaptation/fold 10 test with zero patient overlap; Georgia/CPSC label-independent hash splits; CPSC primary units are complete non-transition 10-second mapped windows", "dataset-specific learning curves with CI plus an unsupported-only exclusion sensitivity", "patient/group or audited CPSC window parent", "Score calibration is separate from frozen-encoder head adaptation; the analysis lock is post-initial-review and is not preregistration; Georgia/CPSC are dataset-specific sensitivity analyses; CPSC is not record-level 27-class or official episode-boundary scoring"),
    ]
    required_artifacts = {
