...
``26_hybrid_morphology_baseline.py`` has produced a manuscript-ready frozen OOF
prediction artifact under the same folds, labels, and threshold protocol.
"""
...
paired_helpers = load_revision_module("15_paired_full_vs_resnet.py", "_hybrid_morphology_paired_helpers")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=PREDICTION_DIR / "hybrid_morphology_oof_predictions.npz",
    )
    parser.add_argument("--freeze-manifest", type=Path, default=MANIFEST_DIR / "oof_final_ema_freeze_manifest.json")
    parser.add_argument(
        "--comparator-summary",
        type=Path,
        default=METRIC_DIR / "hybrid_morphology_baseline_summary.json",
    )
    parser.add_argument(
        "--comparator-manifest",
        type=Path,
        default=MANIFEST_DIR / "hybrid_morphology_baseline_manifest.json",
    )
    parser.add_argument(
        "--out-json",
        type=Path,
        default=METRIC_DIR / "paired_full_vs_hybrid_morphology_comparison.json",
    )
    parser.add_argument(
        "--out-table",
        type=Path,
        default=TABLE_DIR / "table_paired_full_vs_hybrid_morphology.csv",
    )
    parser.add_argument(
        "--out-bootstrap-samples",
        type=Path,
        default=METRIC_DIR / "paired_full_vs_hybrid_morphology_bootstrap_samples.csv",
    )
    parser.add_argument(
        "--out-manifest",
        type=Path,
        default=MANIFEST_DIR / "paired_full_vs_hybrid_morphology_manifest.json",
    )
...
def validate_hybrid_morphology_artifacts(
    *,
    summary_path: Path,
    manifest_path: Path,
    comparator,
    ...
    producer_provenance = validate_baseline_producer_provenance(
        baseline_key="hybrid",
        producer_path=PROJECT_ROOT / "scripts" / "revision" / "26_hybrid_morphology_baseline.py",
    ...
    if comparator.metadata.get("protocol") != EXPECTED_HYBRID_PROTOCOL:
...
def safe_wording(metric_family: str, interpretation: str) -> str:
    if interpretation in {"comparator_nominal_95ci_better", "full_nominal_95ci_better"}:
        favored = (
            "the frozen-transform morphology MLP head"
            if interpretation.startswith("comparator")
            else "Full ECG-RAMBA"
        )
        return (
...
def main() -> None:
    ...
    print(f"Hybrid morphology manifest  : {resolve_path(args.comparator_manifest)}", flush=True)
    ...
    payload = {
        ...
        "inputs": {
            ...
            "hybrid_morphology": hybrid_morphology_info,
        },
    ...
    manifest = {
        ...
        "input_sha256": {
            ...
            "hybrid_morphology_summary": hybrid_morphology_info["summary"]["sha256"],
            "hybrid_morphology_manifest": hybrid_morphology_info["manifest"]["sha256"],
        },
        "artifacts": {
            "json": str(out_json),
            "table": str(out_table),
