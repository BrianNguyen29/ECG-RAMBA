...
def build_checkpoint_contract(args: argparse.Namespace) -> dict:
    ...
    for fold in range(1, 6):
        path = args.checkpoint_dir / f"fold{fold}_{CHECKPOINT_STEM}_final.pt"
...
def require_complete_checkpoint_contract(checkpoint_contract: dict) -> list[dict]:
    """Return ordered checkpoint rows or fail before publishing canonical evidence."""
    rows = sorted(
    ...
    if (
        ...
        raise RuntimeError(
            f"{RUNNER_DISPLAY_NAME} canonical predictions require five exact saved checkpoints; "
...
def prediction_checkpoint_contract_matches(data, checkpoint_contract: dict) -> bool:
    ...
    if not required.issubset(data.files):
        print(
            f"Existing {RUNNER_DISPLAY_NAME} artifact rejected: missing checkpoint provenance "
...
def _project_relative(path: Path) -> str:
    ...
    try:
        ...
        # Durable Colab checkpoints intentionally live in the canonical Drive
        # mirror outside the ephemeral /content/ECG-RAMBA clone.
    ...
    try:
        with np.load(path, allow_pickle=False) as data:
            ...
            if int(npz_scalar(data, "epochs", -1)) != int(args.epochs):
                return None
            if int(npz_scalar(data, "seed", -1)) != int(args.seed):
                return None
...
def main() -> None:
    ...
    if (
        ...
        raise RuntimeError(
            f"{RUNNER_DISPLAY_NAME} canonical run has no complete checkpoint contract. "
