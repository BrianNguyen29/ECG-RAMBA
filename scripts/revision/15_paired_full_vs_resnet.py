"""Paired bootstrap comparison between Full ECG-RAMBA and ResNet1D/CNN.
...
COMPARATOR_LABEL = "ResNet1D/CNN"
PAIRED_INFERENCE_SCHEMA_VERSION = 2
EXPECTED_RESNET_PROTOCOL = "resnet1d_cnn_raw_same_folds_power_mean_v2_q3_threshold_0.5"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=PREDICTION_DIR / "resnet1d_cnn_oof_predictions.npz",
    )
    parser.add_argument("--freeze-manifest", type=Path, default=MANIFEST_DIR / "oof_final_ema_freeze_manifest.json")
    parser.add_argument(
        "--comparator-summary",
        type=Path,
        default=METRIC_DIR / "resnet1d_cnn_baseline_summary.json",
    )
    parser.add_argument(
        "--comparator-manifest",
        type=Path,
        default=MANIFEST_DIR / "resnet1d_cnn_baseline_manifest.json",
    )
    parser.add_argument(
        "--out-json",
        type=Path,
        default=METRIC_DIR / "paired_full_vs_resnet_comparison.json",
    )
    parser.add_argument(
        "--out-table",
        type=Path,
        default=TABLE_DIR / "table_paired_full_vs_resnet.csv",
    )
    parser.add_argument(
        "--out-bootstrap-samples",
        type=Path,
        default=METRIC_DIR / "paired_full_vs_resnet_bootstrap_samples.csv",
    )
    parser.add_argument(
        "--out-manifest",
        type=Path,
        default=MANIFEST_DIR / "paired_full_vs_resnet_manifest.json",
    )
...
def validate_resnet_artifacts(
    *,
    ...
    if not summary_path.exists():
        raise FileNotFoundError(f"Missing ResNet1D/CNN summary JSON: {summary_path}")
    if not manifest_path.exists():
        raise FileNotFoundError(f"Missing ResNet1D/CNN manifest JSON: {manifest_path}")
    summary = json.loads(summary_path.read_text(encoding="utf-8"))
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    producer = PROJECT_ROOT / "scripts" / "revision" / "14_resnet1d_cnn_baseline.py"
    ...
    for source_name, payload in (("summary", summary), ("manifest", manifest)):
        if payload.get("runner_sha256") != producer_sha256:
            raise RuntimeError(f"ResNet1D/CNN {source_name} producer runner SHA is stale.")
    for source_name, payload in [("summary", summary), ("manifest", manifest)]:
        if payload.get("protocol") != EXPECTED_RESNET_PROTOCOL:
            raise ValueError(
                f"ResNet1D/CNN {source_name} protocol mismatch: "
                f"{payload.get('protocol')} != {EXPECTED_RESNET_PROTOCOL}"
            )
        if payload.get("feature_contract") != "raw_ecg_12lead":
            raise ValueError(f"ResNet1D/CNN {source_name} feature_contract must be raw_ecg_12lead.")
...
def validate_pair(full: PredictionSet, comparator: PredictionSet, expected_records: int, expected_classes: int) -> None:
    ...
    if not np.array_equal(full.y_true, comparator.y_true):
        raise ValueError("Full and ResNet1D/CNN y_true arrays differ; paired comparison is invalid.")
    if not np.array_equal(full.record_id, comparator.record_id):
        raise ValueError("Full and ResNet1D/CNN record_id arrays differ; record-level pairing is invalid.")
    if full.class_names != comparator.class_names:
        raise ValueError("Full and ResNet1D/CNN class_names differ.")
    if full.fold_id is not None and comparator.fold_id is not None and not np.array_equal(full.fold_id, comparator.fold_id):
        raise ValueError("Full and ResNet1D/CNN fold_id arrays differ.")
...
def main() -> None:
    ...
    ensure_revision_dirs()
    print("=" * 80, flush=True)
    print("PAIRED FULL ECG-RAMBA VS RESNET1D/CNN COMPARISON", flush=True)
    print("=" * 80, flush=True)
    ...
    print(f"ResNet1D/CNN manifest : {resolve_path(args.comparator_manifest)}", flush=True)
    ...
    print(f"Loaded ResNet1D/CNN: shape={comparator.y_true.shape} sha256={comparator.sha256}", flush=True)

    freeze_info = validate_freeze_manifest(args.freeze_manifest, full, args.expected_checkpoint_kind)
    resnet_info = validate_resnet_artifacts(
    ...
    for metric_index, spec in enumerate(metric_specs(args.threshold, args.n_bins)):
        ...
        row = {
            "comparison": "full_ecg_ramba_vs_resnet1d_cnn",
            "metric": spec.name,
    ...
    payload = {
        ...
        "claim_guidance": {
            "ranking": "Do not claim Full ECG-RAMBA is superior to ResNet1D/CNN on AUROC/AUPRC if paired CIs favor ResNet1D/CNN.",
            "operating_point": "Only claim Full ECG-RAMBA has a better operating point if the paired CI for that metric favors Full; otherwise state that ResNet1D/CNN is stronger.",
        },
    }
    ...
    manifest = {
        ...
        "input_sha256": {
            ...
            "resnet_summary": resnet_info["summary"]["sha256"],
            "resnet_manifest": resnet_info["manifest"]["sha256"],
        },
        "artifacts": {
            "json": str(args.out_json),
            "table": str(args.out_table),
